...
def initialize_adaptor_weights(layer):
    """Initialize weights for the adaptor layers (if required)"""
...
def save_images(original_images, watermarked_images, epoch, folder, resize_to=None):
    ...
    torchvision.utils.save_image(stacked_images, filename, nrow=min(8, stacked_images.shape[0]), normalize=False)
...
def model_from_checkpoint(hidden_net, checkpoint, strict=True):
    """Restores the hidden_net object from a checkpoint object"""
    try:
        # Attempt to load the checkpoint state dict with the option to allow missing keys
        hidden_net.encoder_decoder.load_state_dict(checkpoint['enc-dec-model'], strict=strict)
        
        # Load optimizer state dict, but check if it matches the current model
        try:
            hidden_net.optimizer_enc_dec.load_state_dict(checkpoint['enc-dec-optim'])
        except ValueError as e:
            logging.warning(f"Optimizer state dict mismatch: {e}")
            logging.info("Reinitializing optimizer due to mismatch.")
            # Reinitialize the optimizer here if needed
            hidden_net.optimizer_enc_dec = torch.optim.Adam(hidden_net.encoder_decoder.parameters(), lr=1e-4)
        ...
        if 'encoder.adaptor' in str(e):  # Check if the error is related to missing adaptor layers
            logging.info("Adaptor layers are missing, initializing them for fine-tuning.")
            # Initialize the adaptor layers manually here if needed (e.g., Xavier initialization, etc.)
            hidden_net.encoder_decoder.encoder.adaptor.apply(initialize_adaptor_weights)
        else:
