...
class JpegCompression(nn.Module):
    def __init__(self, quality_factor: int = 50):
        ...
        self.yuv_keep_weighs = self.qf_to_keep_weights(self.quality_factor)
        self.jpeg_mask = None

        # create a new large mask which we can use by slicing for images which are smaller
        self.create_mask((1000, 1000))

    def qf_to_keep_weights(self, qf):
        """
        Map JPEG quality factor (1-100) to number of DCT coefficients to keep.
        Lower QF = fewer kept coefficients = more compression.
        """
        qf = max(1, min(qf, 100))  # Clamp QF
        max_coeffs = 64
        # Linear scaling: more intuitive than logarithmic for now
        y_coeffs = int(np.interp(qf, [1, 100], [5, 64]))  # Y channel
        uv_coeffs = int(np.interp(qf, [1, 100], [2, 32]))  # U and V

        return (y_coeffs, uv_coeffs, uv_coeffs)
    ...
    def apply_conv(self, image, filter_type: str):
        ...
        for channel in range(image.shape[1]):
            ...
            image_yuv_ch = image[:, channel, :, :].unsqueeze(1).clone()
    ...
    def forward(self, noised_and_cover):
        ...
        # Remove Padding (✅ Fix: Return a new tensor instead of modifying in-place)
        processed_image = image_ret_padded[:, :, :image_ret_padded.shape[2]-pad_height, :image_ret_padded.shape[3]-pad_width].clone()
        return [processed_image, noised_and_cover[1]]  # Return new list instead of modifying input